...
public final class ExpressionBuilder {
    ...
    public static Bus buildSOT(CstNode node) {
        ...
        Bus first = buildTerm(sot.first(NonTerminal.Term)
                .orElseThrow(() -> new ConversionException(sot.startOffset(), "SumOfTerms",
                        ConversionException.Reason.MALFORMED_CST,
                        "SumOfTerms sans enfant Term")));
        List<Bus> operands = collectOrOperands(sot.first(NonTerminal.Or_Operand_Star)
                .orElseThrow(() -> new ConversionException(sot.startOffset(), "SumOfTerms",
                        ConversionException.Reason.MALFORMED_CST,
    ...
    private static Bus buildTerm(CstNode node) {
        ...
        Bus first = buildFactor(term.first(NonTerminal.Factor)
                .orElseThrow(() -> new ConversionException(term.startOffset(), "Term",
                        ConversionException.Reason.MALFORMED_CST,
                        "Term sans enfant Factor")));
        List<Bus> operands = collectAndOperands(term.first(NonTerminal.And_Operand_Star)
                .orElseThrow(() -> new ConversionException(term.startOffset(), "Term",
                        ConversionException.Reason.MALFORMED_CST,
    ...
    private static Bus buildFactor(CstNode node) {
        ...
        if (!subset.isVector()) {
            ...
            // produirait un Bus de largeur 0. minIndex/maxIndex normalise pour garantir
            // debut <= fin.
