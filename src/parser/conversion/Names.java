...
public final class Names {
    ...
    public static Subset subsetOf(CstNode signalSubsetOpt) {
        ...
        CstNode hiNode = sub.first(new Terminal(Token.NaturalInteger)).orElseThrow(() -> new ConversionException(
        ...
        CstNode rangeOpt = sub.first(NonTerminal.Range_Opt).orElseThrow(() -> new ConversionException(
                sub.startOffset(), "Signal_Subset_Opt",
                ConversionException.Reason.MALFORMED_CST,
        ...
        CstNode loNode = rng.first(new Terminal(Token.NaturalInteger)).orElseThrow(() -> new ConversionException(
     ...
     *
     * @param moduleNT
     *                     nœud {@code CstInternal(Module)}
     * @return le nom textuel du module
     * @throws ConversionException
     *                                 MALFORMED_CST si le nœud n'est pas un
     *                                 {@code CstInternal(Module)}, ou si
     *                                 l'Identifiant est absent ou non-feuille.
     */
    public static String moduleName(CstNode moduleNT) {
        ...
        CstNode id = mod.first(new Terminal(Token.Identifiant)).orElseThrow(() -> new ConversionException(
    ...
    /**
     * Extrait le {@link SignalRef} (nom + sous-ensemble) d'un nœud NT
     * {@code Signal}.
     */
    public static SignalRef signalRef(CstNode signalNT) {
        ...
        CstNode subsetOpt = sig.first(NonTerminal.Signal_Subset_Opt).orElseThrow(() -> new ConversionException(
                sig.startOffset(), "Signal",
                ConversionException.Reason.MALFORMED_CST,
