...
public final class ModuleCallBuilder {
     ...
     *                           nœud {@code CstInternal(ModuleCall)}
     * @param called
     *                           le module déjà construit qui est appelé
     * @return l'AppelModule correspondant
     * @throws ConversionException
     *                                 si la syntaxe est invalide ou si l'arité ne
     *                                 correspond pas
     */
    public static AppelModule build(CstNode moduleCallNode, Module called) {
        ...
        CstNode firstArgNode = mc.first(NonTerminal.Arg)
                .orElseThrow(() -> new ConversionException(mc.startOffset(), "ModuleCall",
                        ConversionException.Reason.MALFORMED_CST,
        ...
        CstNode sasNode = mc.first(NonTerminal.Separ_Arg_Star)
                .orElseThrow(() -> new ConversionException(mc.startOffset(), "ModuleCall",
                        ConversionException.Reason.MALFORMED_CST,
        ...
        while (!sas.children().isEmpty()) {
            ...
            CstNode separNode = sas.first(NonTerminal.Separ)
                    .orElseThrow(() -> new ConversionException(sasOffset, "Separ_Arg_Star",
                            ConversionException.Reason.MALFORMED_CST,
            ...
            CstNode argNode = sas.first(NonTerminal.Arg)
                    .orElseThrow(() -> new ConversionException(sasOffset, "Separ_Arg_Star",
                            ConversionException.Reason.MALFORMED_CST,
            ...
            CstNode nextSasNode = sas.first(NonTerminal.Separ_Arg_Star)
                    .orElseThrow(() -> new ConversionException(sasOffset, "Separ_Arg_Star",
                            ConversionException.Reason.MALFORMED_CST,
     ...
     *
     * <p>
     * Un Arg valide est un {@code SignalOrLiteralCompound} contenant exactement un
     * {@code Signal} nu (pas de concaténation, pas de littéral).
     * </p>
     *
     * @throws ConversionException
     *                                 MODULE_CALL_INVALID_ARG si l'arg est un
     *                                 littéral ou une concaténation
     */
    private static Descripteur argToDescriptor(CstInternal argNode) {
        // Arg → SignalOrLiteralCompound
        CstNode solcNode = argNode.first(NonTerminal.SignalOrLiteralCompound)
                .orElseThrow(() -> new ConversionException(argNode.startOffset(), "Arg",
                        ConversionException.Reason.MALFORMED_CST,
        ...
        CstNode concatStarNode = solc.first(NonTerminal.Concat_Signal_Or_Litteral_Value_Star)
                .orElseThrow(() -> new ConversionException(solc.startOffset(), "SignalOrLiteralCompound",
                        ConversionException.Reason.MALFORMED_CST,
        ...
        // Vérifier que Signal_Or_Litteral_Value contient un Signal (pas un
        // LiteralValue)
        CstNode solvNode = solc.first(NonTerminal.Signal_Or_Litteral_Value)
                .orElseThrow(() -> new ConversionException(solc.startOffset(), "SignalOrLiteralCompound",
                        ConversionException.Reason.MALFORMED_CST,
        ...
        // C'est un Signal
        CstNode signalNode = solv.first(NonTerminal.Signal)
                .orElseThrow(() -> new ConversionException(solv.startOffset(), "Signal_Or_Litteral_Value",
                        ConversionException.Reason.MALFORMED_CST,
