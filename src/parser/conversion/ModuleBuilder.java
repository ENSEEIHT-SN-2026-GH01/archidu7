...
import simulateur.erwan.AppelModule;
import simulateur.erwan.Descripteur;
...
public final class ModuleBuilder {
    ...
    /**
     * Construit un {@link simulateur.erwan.Module} depuis un nœud CST
     * {@code Module}.
     *
     * @param moduleNode
     *                       nœud {@code CstInternal(Module)} issu du parser
     * @param resolver
     *                       résolveur de modules utilisé pour résoudre les modules
     *                       appelés par les instances {@code $module(...)} /
     *                       {@code module(...)} ;
     *                       fournit la résolution mémoïsée et la détection de
     *                       cycles
     */
    public static Module build(CstNode moduleNode, ModuleResolver resolver) {
        ...
        CstNode instancePlus = mod.first(NonTerminal.Instance_Plus)
                .orElseThrow(() -> new ConversionException(mod.startOffset(), "Module",
        ...
        plan.addAll(buildInstance(
                ip.first(NonTerminal.Instance)
                        .orElseThrow(() -> new ConversionException(ip.startOffset(), "Instance_Plus",
                                ConversionException.Reason.MALFORMED_CST,
                                "Instance_Plus sans enfant Instance")),
                lhsSeen, resolver, branchements,
                freshNames));

        CstNode starNode = ip.first(NonTerminal.Instance_Star)
                .orElseThrow(() -> new ConversionException(ip.startOffset(), "Instance_Plus",
                        ConversionException.Reason.MALFORMED_CST,
        ...
        while (!star.children().isEmpty()) {
            ...
            plan.addAll(buildInstance(
                    star.first(NonTerminal.Instance)
                            .orElseThrow(() -> new ConversionException(starOffset, "Instance_Star",
                                    ConversionException.Reason.MALFORMED_CST,
                                    "Instance_Star non-epsilon sans enfant Instance")),
                    lhsSeen, resolver,
                    branchements, freshNames));
            CstNode nextStarNode = star.first(NonTerminal.Instance_Star)
                    .orElseThrow(() -> new ConversionException(starOffset, "Instance_Star",
                            ConversionException.Reason.MALFORMED_CST,
    ...
    /**
     * Parcourt {@code Param Separ_Param_Star} du nœud Module et construit la
     * {@link Signature}.
     * <ul>
     * <li>Aucun {@code Colon} → tous les params dans {@code entrees},
     * {@code sorties} vide.</li>
     * <li>Un {@code Colon} → params avant → {@code entrees}, après →
     * {@code sorties}.</li>
     * <li>Plus d'un {@code Colon} →
     * {@link ConversionException#MODULE_BAD_SEPARATORS}.</li>
     * </ul>
     */
    private static Signature buildSignature(CstInternal mod) {
        ...
        CstNode firstParam = mod.first(NonTerminal.Param)
                .orElseThrow(() -> new ConversionException(mod.startOffset(), "Module",
                        ConversionException.Reason.MALFORMED_CST,
        ...
        Descripteur firstDesc = Names.descriptorOf(fpInt.first(NonTerminal.Signal)
                .orElseThrow(() -> new ConversionException(fpInt.startOffset(), "Param",
                        ConversionException.Reason.MALFORMED_CST,
        ...
        CstNode spsNode = mod.first(NonTerminal.Separ_Param_Star)
                .orElseThrow(() -> new ConversionException(mod.startOffset(), "Module",
                        ConversionException.Reason.MALFORMED_CST,
        ...
        while (!sps.children().isEmpty()) {
            ...
            CstNode separNode = sps.first(NonTerminal.Separ)
                    .orElseThrow(() -> new ConversionException(spsOffset, "Separ_Param_Star",
                            ConversionException.Reason.MALFORMED_CST,
            ...
            CstNode p = sps.first(NonTerminal.Param)
                    .orElseThrow(() -> new ConversionException(spsOffset, "Separ_Param_Star",
                            ConversionException.Reason.MALFORMED_CST,
            ...
            Descripteur desc = Names.descriptorOf(pInt.first(NonTerminal.Signal)
                    .orElseThrow(() -> new ConversionException(pInt.startOffset(), "Param",
                            ConversionException.Reason.MALFORMED_CST,
                            "Param sans enfant Signal")));
            if (colonSeen) {
                sorties.add(desc);
            ...
            CstNode nextSpsNode = sps.first(NonTerminal.Separ_Param_Star)
                    .orElseThrow(() -> new ConversionException(spsOffset, "Separ_Param_Star",
                            ConversionException.Reason.MALFORMED_CST,
    ...
    private static List<Erwan> buildInstance(CstNode instanceNode, Set<String> lhsSeen,
        ...
        if (inst.has(new Terminal(Token.Dollar))) {
            ...
            CstNode id = inst.first(new Terminal(Token.Identifiant))
                    .orElseThrow(() -> new ConversionException(inst.startOffset(), "Instance",
                            ConversionException.Reason.MALFORMED_CST,
            ...
            CstNode moduleCallNode = inst.first(NonTerminal.ModuleCall)
                    .orElseThrow(() -> new ConversionException(inst.startOffset(), "Instance",
                            ConversionException.Reason.MALFORMED_CST,
        ...
        CstNode opNode = inst.first(NonTerminal.Operation)
                .orElseThrow(() -> new ConversionException(inst.startOffset(), "Instance",
                        ConversionException.Reason.MALFORMED_CST,
        ...
        if (op.has(NonTerminal.ModuleCall)) {
            ...
            CstNode moduleCallNode = op.first(NonTerminal.ModuleCall)
                    .orElseThrow(() -> new ConversionException(op.startOffset(), "Operation",
                            ConversionException.Reason.MALFORMED_CST,
                            "Operation avec ModuleCall sans enfant ModuleCall"));
            return handleModuleCall(moduleCallNode, nom, resolver, branchements,
                    lhsSeen, id.startOffset());
        }
        CstNode subsetNode = op.first(NonTerminal.Signal_Subset_Opt)
                .orElseThrow(() -> new ConversionException(op.startOffset(), "Operation",
                        ConversionException.Reason.MALFORMED_CST,
        ...
        CstNode assignNode = op.first(NonTerminal.Assignment)
                .orElseThrow(() -> new ConversionException(op.startOffset(), "Operation",
                        ConversionException.Reason.MALFORMED_CST,
        ...
        if (assignment.has(NonTerminal.MemoryAssignment)) {
            CstNode memNode = assignment.first(NonTerminal.MemoryAssignment)
                    .orElseThrow(() -> new ConversionException(assignment.startOffset(), "Assignment",
                            ConversionException.Reason.MALFORMED_CST,
                            "Assignment sans enfant MemoryAssignment"));
            return MemoryAssignmentBuilder.build(memNode, nom, lhsSubset, freshNames);
        }
        CstNode sigA = assignment.first(NonTerminal.SignalAssignment)
                .orElseThrow(() -> new ConversionException(assignment.startOffset(), "Assignment",
                        ConversionException.Reason.MALFORMED_CST,
        ...
        CstNode sotc = sigAInt.first(NonTerminal.SumOfTermsCompound)
                .orElseThrow(() -> new ConversionException(sigAInt.startOffset(), "SignalAssignment",
                        ConversionException.Reason.MALFORMED_CST,
     ...
     *
     * <p>
     * Les signaux du circuit appelant pilotés par les sorties de l'appel
     ...
     *
     * @param callOffset
     *                       offset de l'Identifiant du module appelé (diagnostic)
     */
