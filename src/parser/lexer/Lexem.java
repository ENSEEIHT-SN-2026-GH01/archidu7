 ...
 * 
 * Le reste de ce qu'il utilise est seulement utile pour son utilisation dans
 * l'automate:
 * - est ce qu'il est ignoré => est-ce qu'après avoir été trouvé, il est ajouté
 * à la liste des lexèmes
 * - quelle est la priorité de safe => un token est safe si son résultat est
 * ignoré par rapport à d'autres
 * lexèmes lors du match
 * par exemple, on prèfèrerais matcher un module pour "module" plutot qu'in
 * identifiant
 * En plus, une priorité est ajoutée pour classer l'importance des lexèmes safe
 * entre eux
 * 
 */
public class Lexem<T> {
  ...
  /**
   * Construit un lexème par défaut à partir de son token, en spécifiant s'il doit
   * être ignoré
   * 
   * @param token
   *                 le token associé au lexème
   * @param ignore
   *                 {@code true} s'il est ignoré {@code false} sinon
   */
  ...
  public void setSafe(int priority) {
    if (priority < 0)
      throw new IllegalArgumentException("priority doit être supérieur à 0");
   ...
   * 
   * @param indexDepart
   *                      l'index de {@code text} dans le texte
   * @param text
   *                      le text trouvé
   */
