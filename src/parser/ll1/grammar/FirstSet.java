...
public final class FirstSet {
    ...
    public FirstSet(Grammar grammar) {
        ...
        for (NonTerminal nt : NonTerminal.values())
            first.put(nt, EnumSet.noneOf(Token.class));
        compute();
    }
        ...
        while (changed) {
            ...
            for (Production p : grammar.getProductions()) {
                ...
                for (Symbol s : p.getBody()) {
                    ...
                    if (first.get(head).addAll(firstOfS))
                        changed = true;
                    if (!isNullable(s)) {
                        allNullable = false;
                        break;
                ...
                if (allNullable && nullable.add(head))
                    changed = true;
            }
        }
    }
    ...
    private Set<Token> firstOfSymbol(Symbol s) {
        if (s.isEpsilon())
            return Set.of();
        if (s.isTerminal())
            return Set.of(((Terminal) s).getType());
    ...
    public boolean nullable(NonTerminal nt) {
