...
public class FileSimulateur implements Simulateur {
	...
	public FileSimulateur(simulateur.erwan.Module M) {
		this(M.Plan);
		...
		if (M.Branchements != null) {
			...
			for (AppelModule A : M.Branchements) {
				...
				if (A.DE.size() != A.module.Entrees.size())
					throw new RuntimeException("Pas le bon nombre d'entrées durant l'appel du module" + A.module.Nom);
				if (A.DS.size() != A.module.Sorties.size())
					throw new RuntimeException("Pas le bon nombre de sortie lors de l'appel du module " + A.module.Nom);
				...
				for (int curs = 0; curs < A.DE.size(); curs++) {
					...
					if ((De1.nbSignaux() != De2.nbSignaux()))
						throw new RuntimeException(
								"pas le bon nombre de signaux pour l'entree :" + De2.Nom() + ", du module :" + A.module.Nom);
				}
				for (int curs = 0; curs < A.DS.size(); curs++) {
					...
					if (De1.nbSignaux() != De2.nbSignaux())
						throw new RuntimeException(
								"pas le bon nombre de signaux pour la sortie :" + De2.Nom() + ", du module : " + A.module.Nom);
				}
				// On s'est assuré de la bonne formation de l'appel.
			}
			// On s'occupe des sorties de notre module qui sont des entrées pour les modules
			// appelés.
			// SignauxParModule = new ArrayList<>(); //Signaux générées par des modules, pas
			// encores existants, pour des modules
			for (AppelModule A : M.Branchements) {
				Simulateur Smodule;
				if (A.module.Nom.charAt(0) != '$')
					Smodule = new FileSimulateur(A.module);
				else
					Smodule = SimulateurInterne(A.module);
				...
				for (int curs = 0; curs < A.DE.size(); curs++) {
					...
					for (Erwan signal : DS.Erwans()) {
						...
						if (!Dico.existe(signal.Nom()))
							SignauxParModule.add(CS = Dico.getConnecteurE(signal));
						else
							CS = Dico.getConnecteurE(signal).getSignal(Dico);
						BouttonEntree CE = Smodule.getEntrees(curs + 1, curs2 + 1);
						Composant CA = new EntreeModule(CS, CE);
						// System.out.println("\n <>>> Nouveau Branchement : " + CS.getNom() + " de " +
						// M.Nom + " avec " + CE.getNom() + "\n");
						...
						while (!trouve && curseur < SortiesG.size()) {
							// System.out.println(" >> Tentative de match : " +
							// SortiesG.get(curseur).getNom());
		 ...
		 * Il faudrait faire un parcours des entres/sorties des appelmodule pour générer
		 * des liens et structentre correspondants.
		 */
		...
		for (Descripteur DE : M.Sorties) {
			...
			for (String Nom : DE.Noms()) {
				...
				if (!Dico.existe(Nom))
					throw new RuntimeException("Il manque une sortie : " + Nom
			...
			// System.out.println("\nModule : " + M.Nom + "\nSortie ajouté pour
			// l'utilisateur : " + DE.Nom() + " avec " + DE.nbSignaux() + ".\n");
		}
		...
		if (this.ModulesAppeles != null) {
			...
			// Verifier et retirer de EntreeG pour verifier que les signaux sont
			// correctemrnt générés.
			// TODO
			for (int indexModule = 0; indexModule < M.Branchements.size(); indexModule++) {
				...
				// TODO A finir !!!
				for (int curs = 0; curs < A.DS.size(); curs++) { // On parcours les sorties des modules qui sont des entrées de
																													// notre circuit
					...
					for (Erwan signal : DE.Erwans()) {
						...
						if (!Dico.existe(signal.Nom()))
							...
							if (CE.getOrigine() != null)
								throw new RuntimeException("Ce signal est lu et généré ! :" + signal.Nom());
							...
							curs2++;
							// Chercher et retirer les signaux des listes SignauxParModule et
							// EntreeG(entreeModule) pour determiner les signaux non générés.
						}
					}
				}
			}
		...
		for (Descripteur DE : M.Entrees) {
			...
			for (String Nom : DE.Noms()) {
				...
				if (!trouve)
					throw new RuntimeException("Il manque une entree : " + Nom
		...
		/*
		 * for (AppelModule A : M.Branchements) {
		 * if(A.DE.size() != A.module.Entrees.size()) throw new
		 * RuntimeException("Erreur dans un appel module !\n Pb dans le nb d'entrées !"
		 * );
		 * if(A.DS.size() != A.module.Sorties.size()) throw new
		 ...
		 * 
		 * for (Descripteur DE : A.DS) { //Attention, ici on s'occupe des EntresModules,
		 * qui sont des entrés
		 ...
		 * if ((DE.indiceFin() - DE.indiceDebut() + 1) !=
		 * Sinterne.nbSlotSortie(curseurEntree1)) {
		 * throw new
		 * RuntimeException("nb de signaux non correspodants en sortie d'un module");
		 * }
		 ...
		 * System.out.println("Entree retenue : " + SE.getNom() + " avec :" +
		 * SE.getNombre() + " signaux.");
		 * }
		 * for (StructSortie SS : SortiesG){
		 * System.out.println("Entree retenue : " + SS.getNom() + " avec :" +
		 * SS.getNombre() + " signaux.");
		 * }
		 */
	}

	private static Simulateur SimulateurInterne(simulateur.erwan.Module M) {
