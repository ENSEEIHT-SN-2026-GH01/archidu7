...
public class TableauConnecteur {
	...
	public Connecteur getConnecteur(int i) { // TODO On commence le tableau à 1 !!!
		if (i <= 0)
			throw new ErreurIndex(this.connecteurs.length, i, " L'index est trop petit ! ");
		if (i > connecteurs.length)
			throw new ErreurIndex(this.connecteurs.length, i, " L'index est trop grand ! ");
		return connecteurs[i - 1]; // TODO On recupert le cable !!! On peut alterer la valeur
	}

	public void initialiser(String[] Noms) {
		if (Noms.length < connecteurs.length)
			throw new ErreurIndex(this.connecteurs.length, Noms.length, " La liste de nom est trop petite ! ");
		if (Noms.length > connecteurs.length)
			throw new ErreurIndex(this.connecteurs.length, Noms.length, " L'index est trop grand ! ");
