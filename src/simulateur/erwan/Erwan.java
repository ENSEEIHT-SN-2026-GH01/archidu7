 ...
 * <p>
 * Cette classe permet de faire la jonction entre l'interpretation et la
 * simulation.
 * </p>
 *
 * <p>
 * Erwan possède une strucure qui peut être vue arborifique ou recursive :
 * un Erwan contiendra lui-même d'autres Erwan pour décrire un signal dans son
 * ensemble.
 ...
 * Le Erwan "père", soit celui qui n'est pas contenu dans un autre Erwan sera
 * celui de l'affectation,
 * tandis que le "dernier descendant", celui qui ne contient pas d'autre Erwan,
 * sera celui d'une lecture "numérique":
 ...
 *
 * <p>
 * Le langage implémanté pour ce projet comprend la possibilité de créer des
 * "vecteur" de signaux,
 * soit un regroupement cohérant de signaux ayant un seul nom mais etant
 * identifiable par un indice.
 * Ici cela sera modélisé comme des sigaux indépendants,
 * <br>
 * Les vecteur on leur propres methodes statics pour etres généres et
 * manipulées.
 * Il peuvent aussi être atomisés et manipulés indice par indice mais cela
 * demande d'utiliser la variante adaptée.
 * <br>
 * Les méthodes permettant de faire les opération ET et OU sur des vecteur
 * prennent mélangés des signaux et des vecteur,
 * à condition que ceux ci soient dans une liste.
 * Dans ce cas on pourra utiliser la methode toList sur les signaux our les
 * transformer en liste.
 * <br>
 * Certaines methodes de vecteurs sont en double, avec une entree Taille
 * présente ou non.
 * Ces methodes sont parfaitements identique, celle qui n'ont pas le paramettre
 * taille la gère en interne.
 * La variante avec taille permet d'une part de mieux maitriser ce que l'on
 * fait, d'autre part de maximiser la retrocompatinilité.
 * De toute façon la variante sans appelle celle avec.
 * </p>
 *
 * <p>
 * Bon Courage !
...
public class Erwan implements Branchement {
	 ...
	 * Cette methode est utile dans le cas ou l'on souhaite faire des opération
	 * entre des vecteurs et des signaux.
	 * Les methodes traitant les vecteurs prenant en entrée une liste de liste de
	 * signaux,
	 * il faut transformer les signaux en liste de signaux ne comprenant que ce
	 * signal.
	 * 
	 ...
	 * Cette méthode permet de prendre en compte si un signal fait parti d'un
	 * vecteur.
	 * 
	 ...
	 * puisque les autre signaux ont un nom qui dépend exclusivement du nom de
	 * leur(s) entrée(s) et de leur opération.
	 * C'est l'origine de tout signal généré.
	 * 
	 * @param Nom
	 *                 C'est le nom que l'on souhaite donner au signal "copié".
	 * @param Entree
	 *                 De type Erwan, Entree modélise le signal que l'on copie.
	...
	/**
	 * Affectation de la valeur d'un signal appartenant à un vecteur à un autre
	 * signal.
	 * Cela permet de "changer le nom" d'un signal,
	 * puisque les autre signaux ont un nom qui dépend exclusivement du nom de
	 * leur(s) entrée(s) et de leur opération.
	 ...
	 * 
	 * @param Nom
	 *                 C'est le nom du vecteur dans lequel on souhaite intégrer le
	 *                 signal "copié".
	 * @param numero
	 *                 c'est l'indice du signal dans le vecteur.
	 * @param Entree
	 *                 De type Erwan, Entree modélise le signal que l'on copie.
	 * @return La modélisation du signal ainsi copié.
	 */
	 ...
	 * 
	 * @param Entrees
	 *                  Il s'agit de la liste des signaux sur lesquelles on effectue
	 *                  l'operation.
	...
	public static Erwan AND(List<Erwan> Entrees) {
		...
		for (Erwan E : Entrees) {
			String NE = null;
			if (E.Op == Operation.LITTERAL || E.Op == Operation.NOT)
				NE = E.Nom();
			else
				NE = new String("(" + E.Nom() + ")");
			if (Nom == null)
	 ...
	 * 
	 * @param Entree
	 *                 Il s'agit du signal sur lequel on effectue l'operation.
	 * @return La modélisation du signal ainsi généré.
	 */
	public static Erwan NOT(Erwan Entree) {
		...
		String Nom = new String("/"
				+ ((Entree.Op == Operation.LITTERAL || Entree.Op == Operation.NOT) ? Entree.Nom() : "(" + Entree.Nom() + ")"));
	 ...
	 * 
	 * @param Nom
	 *              C'est le nom du signal que l'on souhaite LIRE.
	 * @return La modélisation du signal ainsi généré.
	 */
	...
	/**
	 * Modélisation d'un signal resultant d'une lecture logique d'un signal
	 * appartenant à un vecteur.
	 * 
	 * @param Nom
	 *                 C'est le nom du vecteur dans lequel se trouve le signal que
	 *                 l'on souhaite LIRE.
	 * @param numero
	 *                 c'est l'indice du signal dans le vecteur.
	 * @return La modélisation du signal ainsi généré.
	 ...
	 * 
	 * @param b
	 *            booléen correspondant à la constante voulu.
	 ...
	 * L'idée est la même que pour l'affectation classique mais la logique change un
	 * peu :
	 * Puisque on ne modélise que des signaux et pas de vecteur de signaux, il
	 * faudra récupérer le résultat comme étant une liste de signaux.
	 * On fournit la liste des signaux à intégré au vecteur, le nom du vecteur et
	 * les indice de début et de fin auxquelles on faut l'affectation.
	 * 
	 * @param Nom
	 *                      C'est le nom du vecteur à génerer. (Choix libre)
	 * @param IndiceDebut
	 *                      c'est l'indice du vecteur à partir duquel commence
	 *                      l'affectation.
	 * @param IndiceFin
	 *                      c'est l'indice du vecteur auquel s'arrête l'affectation.
	 * @param Entrees
	 *                      c'est la liste des signaux à intégrer au vecteur.
	...
	public static List<Erwan> ARANGE(String Nom, int IndiceDebut, int IndiceFin, List<Erwan> Entrees) {
		...
		if (Entrees.size() != IndiceFin - IndiceDebut + 1)
			throw new RuntimeException("Nombre non correspondant d'entrées"); // TODO changer l'erreur
		return R;
	}

	/**
	 * Lecture numérique dans le cas de signaux appartenant à un vecteur.
	 * il faut utiliser cette méthode meme si on ne souhaire lire que un seul
	 * signal.
	 * 
	 * @param Nom
	 *                      le nom du vecteur duquel on souhaite lire.
	 * @param IndiceDebut
	 *                      l'indice à partir duquel on souhaite lire.
	 * @param IndiceFin
	 *                      l'indice jusqu'auquel on veut lire, inclu. Attention,
	 *                      dans le cas on l'on ne souhaite lire qu'un signal, c'est
	 *                      égal à IndiceDebut.
	 ...
	 * Cette méthode permet de simplifier l'opération ET bit à bit de deux vecteur,
	 * ou d'un vecteur et d'un signal.
	 * En suposant la taille des vecteur etant de 'n' on peut mettre autant de
	 * vecteur de taille n et 1 que l'on souhaite dans les Entrees.
	 * Un vecteur de taille différente entrainera une erreur.
	 * 
	 * @param Taille
	 *                  C'est la taille des vecteur entre lesquelles on fait
	 *                  l'opération.
	 * @param Entrees
	 *                  C'est la liste des vecteur (eux mêmes des liste de Erwan).
	 * @return La liste des modélisation des signaux inclus dans le vecteur généré
	 *         par l'opération.
	 */
	public static List<Erwan> ANDR(int Taille, List<List<Erwan>> Entrees) {
		...
		for (List<Erwan> e : Entrees) {
			if (e.size() == 1) {
				...
				throw new RuntimeException("Pb de taille"); // TODO changer l'erreur
		}
		for (List<Erwan> L : A) {
			Erwan s = AND(L);
			R.add(s);
	 ...
	 * Similaire à l'autre methode homonime, celle ci gère automatiquement la taille
	 * des vecteur manipulées.
	 * Elle accepte méllangée des signaux (taille 1) et d'autres vecteur d'une autre
	 * taille.
	 * Attention deux vecteur de deux tailles différentes strictement supérieur à 1
	 * entraine une erreur.
	 * 
	 * @param Entrees
	 ...
	 * Cette méthode permet de simplifier l'opération OU bit à bit de deux vecteur,
	 * ou d'un vecteur et d'un signal.
	 * En suposant la taille des vecteur etant de 'n' on peut mettre autant de
	 * vecteur de taille n et 1 que l'on souhaite dans les Entrees.
	 * Un vecteur de taille différente entrainera une erreur.
	 ...
	 * 
	 * @param Taille
	 *                  C'est la taille du vecteur sur lequel on fait l'opération.
	 * @param Entrees
	 *                  C'est le vecteur (lui même une liste de Erwan).
	 * @return La liste des modélisation des signaux inclus dans le vecteur généré
	 *         par l'opération.
	 */
	public static List<Erwan> NOTR(int Taille, List<Erwan> Entrees) {
		if (Entrees.size() != Taille)
			throw new RuntimeException("Pb de taille"); // TODO changer l'erreur
		List<Erwan> R = new ArrayList<>();
		for (int c = 0; c < Taille; c++) {
	 ...
	 *                     l'opération.
	 * @param Constantes
	 *                     C'est la liste des Boolean représentant le vecteur
	 *                     Constant. Attention, un Boolean à null entrainera une
	 *                     erreur.
	 * @return La liste des modélisation des signaux inclus dans le vecteur généré
	 *         par l'opération.
	 */
	public static List<Erwan> CONSTANTER(int Taille, List<Boolean> Constantes) {
		if (Constantes.size() != Taille)
			throw new RuntimeException("Pb de taille"); // TODO changer l'erreur
		List<Erwan> R = new ArrayList<>();
		for (int c = 0; c < Taille; c++) {
			if (Constantes.get(c) == null)
				throw new RuntimeException("Boolean mal initialisé"); /* TODO changer l'erreur */
