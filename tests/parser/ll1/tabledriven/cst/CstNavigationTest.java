 ...
 * (vérifié implicitement par la compilation de ce fichier).
 */
public class CstNavigationTest {

    private static final Production PROD = Grammar.SHDL.getProductions().get(0);

    private CstLeaf leaf(TokenType type, String value, int offset) {
        return new CstLeaf(new Terminal(type), new Token(type, value, offset));
    }

    @Test
    public void first_terminal_present_retourne_match() {
        var l1 = leaf(TokenType.Identifiant, "a", 0);
        var l2 = leaf(TokenType.LeftPar, "(", 2);
        var l3 = leaf(TokenType.Identifiant, "b", 4);
        var node = CstInternal.of(NonTerminal.Module, PROD, List.of(l1, l2, l3));
        var result = node.first(new Terminal(TokenType.LeftPar));
    ...
    public void first_absent_retourne_empty() {
        ...
        var result = node.first(new Terminal(TokenType.RightPar));
        assertFalse(result.isPresent());
    }

    @Test
    public void first_retourne_le_premier_si_plusieurs_matches() {
        var l1 = leaf(TokenType.Identifiant, "x", 0);
        var l2 = leaf(TokenType.Identifiant, "y", 5);
        var node = CstInternal.of(NonTerminal.Module, PROD, List.of(l1, l2));
        var result = node.first(new Terminal(TokenType.Identifiant));
        assertTrue(result.isPresent());
    ...
    public void allOf_retourne_tous_dans_lordre() {
        var l1 = leaf(TokenType.Identifiant, "a", 0);
        var l2 = leaf(TokenType.LeftPar, "(", 2);
        var l3 = leaf(TokenType.Identifiant, "b", 5);
        var l4 = leaf(TokenType.Identifiant, "c", 10);
        var node = CstInternal.of(NonTerminal.Module, PROD, List.of(l1, l2, l3, l4));
        var result = node.allOf(new Terminal(TokenType.Identifiant));
    ...
    public void allOf_aucun_match_renvoie_liste_vide() {
        var l1 = leaf(TokenType.Identifiant, "a", 0);
        var node = CstInternal.of(NonTerminal.Module, PROD, List.of(l1));
        var result = node.allOf(new Terminal(TokenType.RightPar));
    ...
    public void has_terminal_oui_non() {
        ...
        var l2 = leaf(TokenType.LeftPar, "(", 2);
        var node = CstInternal.of(NonTerminal.Module, PROD, List.of(l1, l2));
        assertTrue(node.has(new Terminal(TokenType.Identifiant)));
        assertFalse(node.has(new Terminal(TokenType.RightPar)));
    }

    @Test
    public void first_avec_NonTerminal() {
        // Un internal contenant un autre CstInternal (NT Module)
        var leaf = leaf(TokenType.Identifiant, "x", 0);
        var inner = CstInternal.of(NonTerminal.Module, PROD, List.of(leaf));
        // On crée un faux leaf supplémentaire pour que outer ait >1 enfant avec offsets valides
        var leaf2 = leaf(TokenType.Identifiant, "y", 3);
    ...
    public void leaf_first_toujours_empty_independamment_du_symbol() {
        var l = leaf(TokenType.Identifiant, "x", 0);
        // first toujours vide
        assertFalse(l.first(new Terminal(TokenType.Identifiant)).isPresent());
        assertFalse(l.first(NonTerminal.Module).isPresent());
        // allOf toujours vide
        assertTrue(l.allOf(new Terminal(TokenType.Identifiant)).isEmpty());
        // has toujours false
        assertFalse(l.has(new Terminal(TokenType.Identifiant)));
