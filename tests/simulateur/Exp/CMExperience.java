...
public class CMExperience {
	...
	public static void main(String[] Args) {
		...
		S.add(DS);
		simulateur.erwan.Module M = new simulateur.erwan.Module("Expérience 6", PlanCircuitSousModule, E, S, null);
		...
		DEM.add(new Descripteur("Entree", 1, 2));
		DSM.add(new Descripteur("Inter", 0, 3));
		AMM.add(new AppelModule(M, DEM, DSM));
		List<Erwan> EMM = new ArrayList<>();
		EMM.addAll(AR("Sortie", 0, 3, RANGE(ER(4, LR(NR(1, LR("Entree", 0, 0)), LR("Inter", 0, 3))))));
		EMM.addAll(AR("Sortie", 4, 7, RANGE(ER(4, LR(LR("Entree", 0, 0), LR("Inter", 0, 3))))));
		DEM = new ArrayList<>();
		DSM = new ArrayList<>();
		DEM.add(new Descripteur("Entree", 0, 2));
		DSM.add(new Descripteur("Sortie", 0, 7));
		Simulateur Si = new FileSimulateur(new simulateur.erwan.Module("Docode3to8", EMM, DEM, DSM, AMM));
