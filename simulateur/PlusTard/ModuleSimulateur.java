...
public class ModuleSimulateur implements Simulateur{
	...
	public ModuleSimulateur(erwan.Module M){
		...
		if (M.Branchements != null) {
			...
			for (AppelModule A : M.Branchement) {
				//Vérification des formats entrées et sorties
				if (A.DE.size() != A.module.Entrees.size()) throw new RuntimeException("Pas le bon nombre d'entrées durant un appel module");
				if (A.DS.size() != A.module.Sorties.size()) throw new RuntimeException("Pas le bon nombre de sortie lors d'un apppel module");
				...
				for (int curs = 0; curs < A.DE.size(); curs ++) {
					...
					if ((De1.nbSignaux() != De2.nbSignaux()) throw new RuntimeException("pas le bon nombre de signaux pour l'entree :" + De2.Nom() + ".");
				}
				for (int curs = 0; curs < A.DS.size(); curs ++) {
					Descripteur De1 = A.DS.get(curs);
					Descripteur De2 = A.module.Sorties.get(curs);
					if ((De1.nbSignaux() != De2.nbSignaux()) throw new RuntimeException("pas le bon nombre de signaux pour la sortie :" + De2.Nom() + ".");
				}
